...
and \textit{Docker Compose} itself \cite{docker} \cite{docker-compose} ---
...
One has to create a class annotated with \code{@RelationshipEntity("CALLS")}
...
Just like in the above example, \code{"CALLS"}, \code{"OWNS"}, \code{"KNOWS"} would
...
\end{lstlisting}
\normalsize
The \code{@Id} is required once again, apart from that the \code{@Relationship}
...
\caption*{
  ...
  two \code{Unit} \textit{Nodes} together seen in figure \ref{fig:data-models-nodes}.
}
...
\code{Unit} \textit{Nodes} are linked to each other and in which direction.
...
The first resource only accepts \code{POST} requests and is solely used to
...
\caption*{
  Exhibits the data that resource 1 (\code{POST "/analysis/"}) mandates to function.
}
...
\begin{tabularx}{\textwidth}{|Y|Y|Y|}
 ...
 \hline\hline
 \code{staticAnalysisFile}         & \code{.jar} / \code{.zip} &
 Either a \code{.jar} file with compiled bytecode or a \code{.zip} file with plain source code;
 both are used to build the static call graph, either via compiled code or by traversing the source code \\\hline
 \code{dynamicAnalysisFile}        & \code{.jfr} &
 ...
 recorded \code{.jfr} files from within another Java application \\\hline
 ...
 The format is depending on which \code{vcsPlatform} was chosen; a log file of the repository of that platform
 of the application as a plain text file \\\hline
\end{tabularx}
\caption{Description of the binary data required by resource 1}
\caption*{
  Exhibits the binary data that resource 1 (\code{POST "/analysis/"}) mandates to function
...
The second resource, \code{GET "/analysis/\{projectName\}"}, is solely used to retrieve a project
that was persisted earlier via resource 1. Hence, it only mandates the \code{projectName} as a string
...
The \code{Clusterer} class is the entry point for clustering an input graph using any of
the available algorithms. The graph that was retrieved out of \textit{Neo4j} is passed into
the \code{Clusterer} after being cloned owing to the fact that by default, all of the available
...
the \code{Clusterer} should apply to the input graph.
...
\begin{enumerate}
  ...
  \item The rendered graph either plain or clustered; every cluster then depicts
        ...
        examined. Moreover, a completely textual view can alternatively be
        displayed.
...
\caption*{
  The front-end offers a graph view of the clustered graph that depicts
...
rendered with a lower frame rate. The rendering is done via a HTML5 canvas.
...
arrow heads depicting in which directions the microservice would invoke
...
as well as the full qualified name and the byte size when hovering over it
...
other microservice recommendations.
...
\paragraph{Un-clustered mode}
A last, additional graph view mode renders the graph in its un-clustered form
as it is persisted in \textit{Neo4j}. This mode is mainly interesting to
examine the structure or architecture of the input monolithic application.
The mode can be seen in figure \ref{fig:un-clustered-graph-view} and is
activated by toggling the \textit{Clustered View} option which also disables
the \textit{Hide Inter-Cluster Edges} as well.

\begin{figure}[htbp]
\centering
\includegraphics[width=.96\textwidth]{un-clustered-graph-view}
\caption{A closeup of the un-clustered graph view mode}
\caption*{
  The graph view has a third mode that can be used to see the un-clustered
  graph as it is persisted in \textit{Neo4j}. This can be used to examine
  the structure of the input monolithic application on its own.
}
\label{fig:un-clustered-graph-view}
\end{figure}
...
\caption*{
  Additionally to the \textit{graph view}, a solely textual view was implemented
  that just displays which units should belong to which service.
}
