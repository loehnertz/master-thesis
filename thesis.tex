...
\label{algo:class-walk}
  ...
  \BlankLine
  push the classes containing \code{main} methods of the application onto queue $cq$\;
  \BlankLine
...
describe the type of relation \cite{neo4j-ogm}. Thus, \code{"CALLS"},
\code{"OWNS"}, \code{"KNOWS"} would be good examples. Due to this implementation
...
the class needs to encompass three properties. Namely, these are \code{@StartNode},
\code{@EndNode}, and \code{@Id}. The annotations \code{@StartNode}
...
from the outside, although it is advised just to leverage the \code{@GeneratedValue}
...
\textit{Neo4j}, these directions are ignored via the \code{.equals()} and
\code{hashCode()} methods of the custom data class representing an edge of a graph.
...
save()} method (e.g., a \textit{Node} connected to another one via a
...
will not change in any case. The \code{Unit} \textit{Node} is the main \textit{Node}
...
table \ref{table:data-resource-1} have to be in the example case that \code{"JAVA"}
was chosen as the \code{projectPlatform}. Finally, we added a feature
...
all of the reference implementations in C or C++ are designed as
\textit{Command Line Interfaces} (CLI) instead of libraries. As a consequence,
in the latest version of \textit{Steinmetz}, we opted to wrap the CLI with the
\code{ProcessBuilder} class of the Java platform \cite{process-builder}.
This enables us to call any CLI from within a Java application by invoking a process,
piping input into \code{STDIN}, and parsing the output of the invoked program from the
...
executes multiple instances of it concurrently, leveraging the
\textit{coroutine} feature of the Kotlin programming language.
...
\code{GraphConverter} and \code{GraphInserter}, which are handling the
...
\code{Platform} enum. In this chapter, the exact implementation of all of these
...
\code{extraction.coupling} package contains sub\hyp packages for every coupling
...
then allows us to ignore it in the analysis via the \code{basePackageIdentifier}
