...
The second resource, \code{GET "/analysis/\{projectName\}"}, is solely used to
...
Three freely available open-source applications were chosen to be run through
\textit{Steinmetz} to generate a set of microservice recommendations,
to determine which graph clustering algorithm performed the best in the case
of each input application and also overall, and if the microservice candidate
recommendations yielded good results regarding the devised metrics.


\subsection{Subjects}

As shown in section \ref{sect:background-architecture}, a microservices\hyp based
architecture only makes sense for large scale web-based back-end applications
compared to libraries, SDKs, and dedicated GUI software.
...
\begin{itemize}[noitemsep]
    \item Dynamic coupling: Still being able to execute in the latest version
...
% Finally, the following three applications that fulfill both of these requirements
% were selected after an intensive survey of possible ones via the search functionality
% of GitHub and regular searches via Google:

% \begin{itemize}[noitemsep]
%     \item Heritrix (\url{https://github.com/internetarchive/heritrix3})
% \end{itemize}
