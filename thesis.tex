...
The listing \ref{git-log} depicts this with one example commit entry
out of a large Git log of a real application. However, multiple of those
commit entries would be analyzed to calculate \textit{evolutionary similarity}.

Precisely, over the duration of a certain time period \([t_1, t_2]\)
or the entirety of the log \([t_{min}, t_{max}]\), every commit \(c \in C\)
contains at least one entity (i.e. file) out of the code base at that point
in time \(\exists e \in c, e \in E\) that was modified
(including creation and deletion) as part of the commit.
Over the course of all the commits \(\forall c \in C\) in a given time period
\([t_1, t_2]\), the commits where an entity \(e_1 \in E\) changed together
with another entity \(e_2 \in E\) are gathered:
\[
  C' = \{ c \in \{ c \in C \mid c \in [t_1, t_2] \} \mid e_1 \in c \land e_2 \in c \}
\]
Furthermore, the ratio of how many of the total commits that modified
the entity \(e_1\) is calculated to obtain a percentage:
\[
  \frac{\vert C' \vert}{\{ c \in \{ c \in C \mid c \in [t_1, t_2] \} \mid e_1 \in c \}}
\]
As a consequence, the time interval \([t_1, t_2]\) covering the commits can be
shifted such that either every commit is evaluated on its own or that multiple
are evaluated at a time.

Additionally, a threshold can be applied to filter out noise such that only
after a certain amount of shared commits of two entities \(e_1 \in E, e_2 \in E\),
the commits are added to the shared commits \(C'\) to increase the relevance
...
for instance \cite{newman2015building}.
...
of this mapping as part of our methodology are discussed in chapters
\ref{chap:extracting-coupling} and \ref{chap:software-as-weighted-graph}.

\begin{figure}[htbp]
\centering
\includegraphics[width=.90\textwidth]{principles-of-microservices}
\caption{The mapping of the principles of microservices to coupling types}
...
would make a combined analysis on all of them non-functional.
