...
\newcommand{\thesisdate}{26/08/2020}
\newcommand{\thesislocation}{Amsterdam}
\newcommand{\firstmarker}{Dr.\ Ana Maria Oprescu}
...
The microservices paradigm gains more and more traction, and
many companies choose to adopt it for new developments as well as to transform
existing monolithic software to be microservices\hyp based instead. However,
the latter approach bears many difficulties that involve much manual work,
specifically in the early stages of such a decomposition process. Determining
the boundaries between the services, where one service ends, and the next one
begins, is a notoriously difficult task for which a lot of literature exists
that however mainly helps with providing a framework to determine said
boundaries manually, with \textit{Domain-driven design} by Eric Evans laying
the foundation for this entire domain of research.

This thesis devises a methodology that automatically generates microservice
candidate recommendations for any given piece of monolithic software. To do
this, four dimensions of coupling are calculated out of the input software that
each construct a weighted graph with the edges resembling coupling between the
units of the software. Next, the four graphs are merged into one before being
clustered by specifically surveyed and assessed state-of-the-art graph
clustering algorithms --- the resulting clusters then resemble the microservice
candidate recommendations. Additionally, a large set of numeric metrics was
formulated to evaluate the output of the methodology. Finally, the devised
methodology was fully implemented, with an additional graphical user interface
for ease of use, to work with input Java software while being easily extensible
to be able to support other platforms in the future.
...
around it gets larger and larger every day. One of the terms that gained a lot
of traction in recent years is \textit{microservices}. Many engineers talk
about their benefits and challenges but the general notion is positive and the
buzz is undeniable. At the end of the day, microservices are nothing more than
an architectural pattern for designing back-end software. For those that
actually take the step to build and deploy such an application landscape, the
journey is not always straightforward. The idea is still evolving and mandates
many new strategies and concepts that developers and operations have to deal
with when choosing to devote their business logic to leverage microservices.
...
description in the \textit{\nameref{chap:related}} chapter, the approach itself
...
\code{public}, \code{static}, \code{void}) appear in every source code document
...
As part of our \textit{\nameref{chap:evaluation}}, we conduct
  ...
  \While{class queue $cq$ is not empty}{
    ...
    \ForEach{method $vm$ of class $vc$}{
      \ForEach{class $ic$ instantiated or invoked from method $vm$}{
        add pair of $vc$ and $ic$ to call graph list $cg$ while recording their $RFC_0$ value\;
...
specific keywords such as \code{package} or \code{import} trigger that the line
gets discarded.
...
i.e., \code{W} from regular expressions), which is every character but a-z, A-
...
white space characters (i.e., \code{t} from regular expressions)
...
Finally, as discussed in subsection \ref{subsect:evolutionary-similarity}, we
use a threshold for this extraction step to filter out noise. The values are
constants that can be changed while the defaults that we employ are:
\begin{itemize}[noitemsep]
...
timeline to show how the \textit{evolutionary coupling} is calculated via a
given VCS log while the direction of the coupling relationship is taken that
was extracted out of the static analysis into a directed graph.
...
\textit{\nameref{chap:background}} chapter together with a range of available,
renowned algorithms.
...
\begin{tabularx}{\textwidth}{|Y||p{2.4cm}|Y|Y|Y|Y|Y|Y|Y|Y|Y|}
 \hline
 \rot{Citations~} & \rot{Runtime complexity~} & \rot{Implementation available~} & \rot{Does not require number of clusters~} & \rot{Deterministic~} & \rot{Considers weighted edges~} & \rot{Hard clustering~} & \rot{Hierarchical~} \\
 \hline\hline
...
containers via \textit{Docker Compose} which allows for easy boot-up and tear-
...
The back-end is written in Kotlin, runs as a server process, and exposes a REST-
API that the front-end will use and consume to upload and fetch information.

The database is persisting data that needs to be available even when the back-
...
One has to create a class annotated with \code{
@RelationshipEntity("")}, the annotation itself is part of the OGM library and
...
describe the type of relation \cite{neo4j-ogm}. Thus, \code{"CALLS"}, \code{
"OWNS"}, \code{"KNOWS"} would be good examples. Due to this implementation
...
the class needs to encompass three properties. Namely, these are \code{
@StartNode}, \code{@EndNode}, and \code{@Id}. The annotations \code{@StartNode}
and \code{@EndNode} need to point to a non-nullable instance of a \textit{Node}
...
class to the two vertices that it connects. The \code{@Id} annotation
...
from the outside, although it is advised just to leverage the \code{
@GeneratedValue} annotation that will handle all this mapping automatically
...
\code{@Property} annotation since \textit{Neo4j} can persist any data together
...
are also created by annotating a class, this time with the \code{@NodeEntity}
...
\end{lstlisting}
The \code{@Id} is
required once again while the \code{@Relationship} annotations are not
...
\textit{Neo4j}, these directions are ignored via the \code{.equals()} and \code{
.hashCode()} methods of the custom data class representing an edge of a graph.
...
A \code{SessionFactory}
that is shipped with the OGM library returns \code{Session} objects that among
...
annotated with the \code{@NodeEntity} or \code{@RelationshipEntity} annotation
...
recursively persist the data attached to the object passed into the \code{
save()} method (e.g. a \textit{Node} connected to another one via a \textit{
...
only makes use of the filtering features, such as \code{session.loadAll(
Unit.class, Filter("projectName", ComparisonOperator.EQUALS, "test"))}. Once
...
Precisely, the \code{Metrics} \textit{Node} persists metrics on a per\hyp
...
will not change in any case. The \code{Unit} \textit{Node} is the main \textit{
...
\ref{chap:extracting-coupling}. The \code{CALLS} \textit{Relationship}
is used to persist which \code{Unit} \textit{Nodes} are linked to each other.
...
The second resource only accepts \code{POST}
...
table \ref{table:data-resource-1} have to be in the example case that \code{
"JAVA"} was chosen as the \code{projectPlatform}. Finally, we added a feature
...
(specifically the \code{dynamicAnalysisFile}) while the analysis can still be
...
The third resource, \code{GET "/analysis/\{projectName\}"},
is solely used to retrieve a project that was persisted
earlier via resource 2. Hence, it only mandates the \code{projectName} as a
...
Technically, only one logic controller exists, the \code{AnalysisController},
...
containing the logic. The \code{AnalysisController} class implements the actual
controller for all of the \code{"/analysis"} resources covered in subsection
\ref{subsect:api-resources}. From there, the \code{analysis} controller package
...
The \code{Clusterer} class is the entry point for
clustering an input graph using any of the available algorithms. The graph that
was retrieved out of \textit{Neo4j} is passed into the \code{Clusterer} after
...
identify which graph clustering algorithm the \code{Clusterer} should apply to
the input graph.
...
implements the \code{ClusteringAlgorithmManager} interface. In these classes,
...
Some of the reference implementations are written in Java, which allows us to
integrate them into our implementation directly. However, the majority of them
are compiled C or C++ programs that are more difficult to integrate. Initially,
we leveraged the \textit{Java Native Interface} (JNI), a foreign function
invocation interface bundled with the Java platform, that can be used to
integrate C, C++, and assembly programs into a Java application \cite{jni}. On
the downside, the source code of the C and C++ programs has to be modified to
utilize the JNI. However, we did not have access to all of the reference
implementations as source code; some were only shipped pre-compiled. Moreover,
all of the reference implementations in C or C++ are designed as \textit{
Command Line Interfaces} (CLI) instead of libraries. As a consequence, in the
latest version of \textit{Steinmetz}, we opted to wrap the CLI with the \code{
ProcessBuilder} class of the Java platform \cite{process-builder}. This enables
us to call any CLI from within a Java application by invoking a process, piping
input into \code{STDIN}, and parsing the output of the invoked program from the
processes \code{STDOUT}. This might seem error-prone at first sight. However,
the CLIs are all well-defined, which makes input and output of them easy to
encode and decode. Additionally, we added a lot of validation for the input
such that we have not seen an error or malfunction when testing and evaluating
our implementation so far. The only downside is that the portability of the
Java platform is lowered a bit, as the compiled native programs are platform-
specific. Nevertheless, the same problem is present when using JNI.

Furthermore, some graph clustering algorithms mandate multiple runs to yield
good results. Thus, an additional executor class is utilized there which
contains the actual logic while the manager class wraps the executor class and
executes multiple instances of it concurrently, leveraging the \textit{
coroutine} feature of the Kotlin programming language.


\subsubsection{Extraction}
The sub-packages within the package \code{extraction.coupling}
...
Firstly, the sub-package \code{extraction.graph} contains two classes, \code{
GraphConverter} and \code{GraphInserter}, which are handling the
...
the \textit{Java} platform. The \code{Platform.kt} file contains an enum with
all of the available platforms --- currently only \code{JAVA}. However, later
...
The \code{AbstractExtractor} and its \code{ExtractorCompanion} are an
...
e.g., \code{AbstractDynamicAnalysisExtractor}). The abstract class itself,
however, implements the \code{AbstractExtractor}. Within each of the sub-
packages, there are sub-packages for each platform available in the \code{
Platform} enum. In this chapter, the exact implementation of all of these is
...
The \code{main} method accepts a list of documents as plain-text strings, the
...
analyzes. As seen in figure \ref{fig:coupling-extraction-structure}, the \code{
extraction.coupling} package contains sub\hyp packages for every coupling
...
analysis of software written in Java, hence the \code{java} packages in each
\code{platforms} sub\hyp package. To be able to analyze another language or
...
lower frame rate. The rendering is done via a HTML5 \code{<canvas>}.
...
to five \code{NOT} operators. Thus, this query still yields good results
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.96\textwidth]{results/experiment_4-1}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.96\textwidth]{results/experiment_4-2}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.96\textwidth]{results/experiment_4-3}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.96\textwidth]{results/experiment_4-4}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.96\textwidth]{results/experiment_4-5}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.96\textwidth]{results/experiment_4-6}
...
Finally, we decided to look into the question how the different
...
Clustering the weight variable combination that lead to the lowest average
objective values resulted in a clustering conisting of double the
...
\hline
Solution & Static Variable & Semantic Variable & Evolutionary Variable & Static Objective & Semantic Objective & Evolutionary Objective & Average Objective & Amount of Clusters & Ratio of Inter\hyp Cluster Edge Weights \\
\hline\hline

Lowpoint  & $0.0029583$ & $0.0000002$ & $0.6483049$ & $0.4203270$ & $0.4335106$ & $0.8548357$ & $0.5695578$ & $90$ & $10\%$ \\\hline
Midpoint  & $0.4035683$ & $0.0144182$ & $0.9301752$ & $0.4752754$ & $0.4971534$ & $0.8439260$ & $0.6054516$ & $65$ & $22\%$ \\\hline
Highpoint & $0.7901847$ & $0.3441048$ & $0.8325046$ & $0.5013523$ & $0.5657572$ & $0.8033364$ & $0.6234819$ & $44$ & $31\%$ \\\hline

\end{tabularx}
\caption{The results of the low, mid, and highpoint of the range of Pareto\hyp optimal solutions, on the example of the \textit{OpenCMS} input project}
...
then allows us to ignore it in the analysis via the \code{
basePackageIdentifier} parameter. Hence, in an actual microservice
...
corroborate the findings of our \textit{\nameref{chap:evaluation}} as part of
